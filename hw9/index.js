//1.Ստեղծել կլաս , կանստրուկտորի միջոցով   տալ number հատկություն: Ստեղծել այդ կլասում մեթոդը,
// որը  կանչելիս  այդ թիվ հատկության արժեքից կհանի  այցելուի մուտքագրած  թիվը:2
//
const num = 12;
...
const n1 = new Index(22)
console.log(n1.hanum(num));
// 2. Ստեղծել կլասս , որին կանստրուկտրով տալիս են  էլ. հասցե հատկությունը: Էլ. հասցեն  պետք է լրացվի
// set մեթոդներով՝ստուգելով արդյոք  տրվող արժեքը վերջանում է mail.ru,gmail.com, rambler.ru կամ yahoo.com-ով ,
// նոր վերագրել այդ արժեքը: Ստեղծել get մեթոդ, որ թույլ կտա արտածել mail հատկության արժեք եթե կա արժեք կամ արտածի "No" :
//
class Email {
    constructor(el) {
        this.el = el
    ...
    setemail(email) {
        const dom = ["mail.ru", "gmail.com"].some(x => {
            if (email.endsWith(x)) {
                console.log("Yes")
                return true
            }
...
const em1 = new Email("viki@mailk.ru")

console.log(em1.el);
// 3. Ստեղծել Person, Task և ToDoList կլասներ ։ Person-ը ունի id (չկրկնվող արժեք), name հատկությունները ։
// Task ունի id (չկրկնվող արժեք), text , assigned, log, և status հատկությունները ։ text-ը պարդադիր պետք է փոխանցվի task ստեղծելուց։
// assigned-ը սկզբից դատարկ է , իսկ status-ը կարող է լինել միայն ['todo', 'in development', 'testing', 'done'] արժեքներից որևէ մեկը,
//  սկզբից 'todo'է։ log-ում պահվում է ժամանակը և ստատուսը, որը վերագրվել է status հատկությանը։
...
class Task {
    constructor(id) {
        ...
        this.assigned = ""
        this.log = log
        this.status = "todo"['todo', 'in development', 'testing', 'done']
    }
